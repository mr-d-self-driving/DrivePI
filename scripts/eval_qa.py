...
def parse_args():
    ...
    # New parameter: test only the projector
    parser.add_argument(
        '--projector_only',
        action='store_true',
    ...
    # New parameter: projector weights file path
    parser.add_argument(
...
class ProjectorOnlyChatbot:
    """Initialize the model as in training, then only load trained weights for the projector"""
    ...
    def __init__(
        ...
        # Initialize the complete model as in training
        from emova.model.language_model.builder import build_language_model

        # Build language model (as in training)
        ...
        # Initialize vision modules (as in training)
        ...
        if self._tokenizer is None:
            # If no get_tokenizer method, try to get from config
    ...
    def chat(self, inputs) -> List[str]:
        """Same chat method as Chatbot, supporting full QA functionality"""
        image_sizes = None
    ...
    def chat(self, inputs) -> List[str]:
        ...
        if hasattr(results, 'sequences'):
            ...
            if hidden_states is not None:
                ...
                # each element is also a tuple, containing hidden states for all layers
...
if __name__ == '__main__':
    ...
    for task_file in ['nusceneqa_val_llava.json']:
        ...
        if is_train_format:
            ...
            for item in data:
                new_item = {
                    ...
                    "image": os.path.join("/path/drivePI_data/unilion_bev_feats_val/", item['history']['token'] + '.npy'),
        ...
        # Choose which mode to use based on arguments
