...
class Chatbot(object):
    ...
    def chat(self, inputs) -> List[str]:
        ...
        if isinstance(inputs[0]['image'], str) and inputs[0]['image'].endswith('npy'):
            # batched_visuals = [np.load(item['image']) for item in inputs]  # [B, N]
            # item['image'] = item['image'].replace('/data/', '/data3/').replace('/code/', '/')
            
            # batched_visuals = [np.load(item['image'].replace('/data/', '/data3/').replace('/code/', '/')) for item in inputs]  # [B, N]
        ...
        results = self.model.generate(
            ...
            top_p=gen_kwargs["top_p"],
            num_beams=gen_kwargs["num_beams"],
            max_new_tokens=gen_kwargs["max_new_tokens"],  ###最大输出50个tokens！！！！ zliu: 50
            use_cache=self.use_cache,
            output_hidden_states=True,  # 显式请求隐藏状态
        ...
        if hasattr(results, 'sequences'):
            ...
            if hidden_states is not None:
                ...
                # hidden_states 是一个元组，每个元素对应一个生成步骤
                # 每个元素又是一个元组，包含所有层的隐藏状态
                if hidden_states and hidden_states[0]:
                    last_layer_hidden = hidden_states[0][-1]  #第一步的最后一层
                    print(f"input Last hidden state shape: {last_layer_hidden.shape}")

                    ###################
                    # save_dir = './unilion_bev_feats_train/'
                    # os.makedirs(save_dir, exist_ok=True)
                    # single_output = last_layer_hidden.detach().cpu()
                    #
                    # # 保存为 .bin 文件
...
if __name__ == '__main__':
    ...
    task_file_root = '/path/drivepi_captions/captions_4d_mllm_all_11_9/occ/'
    for task_file in ['occ_yorn_val_500_fix.json', 'occ_class_val_500_fix.json']:
        ...
        if accelerator.is_local_main_process:
            ...
            with open(os.path.join(chatbot.output_dir, task_file.replace('.json', '_val.json')), 'w', encoding='utf-8') as f:
    ...
    if accelerator.is_local_main_process:
        for task_file in ['occ_yorn_val_500_fix.json', 'occ_class_val_500_fix.json']:
            data = json.load(open(os.path.join(chatbot.output_dir, task_file.replace('.json', '_val.json'))))
            correct_num = 0
            for item in data:
                if item['pred'].lower() == item['answer'].lower():
                    correct_num += 1
