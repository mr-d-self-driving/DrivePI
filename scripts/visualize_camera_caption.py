...
def parse_args():
    ...
    parser.add_argument(
        '--json-path', 
        type=str, 
        default='/data/runhui/logdir/emova_llava-qwen2_5-3b-bev-feature-pretrain/nuscenes_val_100_generated_caption.json', 
...
def load_data_from_path(json_path):
    ...
    data = [item for item in data if item and 'camera_paths' in item]
    if not data:
...
def get_item_data(index, data, camera_root):
    ...
    caption = item.get('answer', 'No caption found for this item.')
    ...
    return stitched_image, caption, info_text, index
...
def create_ui(initial_data, camera_root):
    ...
    with gr.Blocks(theme=gr.themes.Soft()) as demo:
        ...
        gr.Markdown("Click the button to view a new random sample from the dataset, or enter a specific index.")
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                index_input = gr.Number(
                    label="Enter Index (0 to {})".format(len(initial_data) - 1),
                    minimum=0,
                    maximum=len(initial_data) - 1,
                    value=0,
                    step=1
                )
                index_button = gr.Button("📋 Show Specific Index", variant="secondary")
                gr.Markdown("---")
        ...
        def show_random_sample(data, camera_root):
            if not data:
                ...
                return placeholder, "No data loaded.", "No data loaded.", 0
            
            random_index = random.randint(0, len(data) - 1)
            return get_item_data(random_index, data, camera_root)

        def show_specific_index(index, data, camera_root):
            if not data:
                placeholder = Image.new('RGB', (800, 225), color='grey')
                draw = ImageDraw.Draw(placeholder)
                draw.text((10, 10), "No data loaded.", fill='red')
            ...
            return get_item_data(index, data, camera_root)

        # Connect the random button to the function
        random_button.click(
            show_random_sample,
            inputs=[data_state, camera_root_state],
            outputs=[image_display, caption_display, info_display, current_index_display]
        )
        
        # Connect the index button to the function
        index_button.click(
            show_specific_index,
            inputs=[index_input, data_state, camera_root_state],
            outputs=[image_display, caption_display, info_display, current_index_display]
        )
