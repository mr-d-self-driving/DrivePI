...
def process_one_sample(sem_pred, lidar_rays, output_origin, occ_class_names, instance_pred=None, flow_pred=None):
    ...
    for t in range(T):
        ...
        if flow_pred is not None:
            pred_flow = flow_pred[coord_index[:, 0], coord_index[:, 1], coord_index[:, 2]]  # [N, 2]
            pred_pcds = torch.cat([pred_label.float(), pred_dist.float(), pred_flow], dim=-1)
        else:
        ...
        if instance_pred is not None:
            ...
            pred_pcds = torch.cat([pred_pcds, pred_instance.float()], dim=-1)
...
def calc_metrics(pcd_pred_list, pcd_gt_list, occ_class_names, with_ave=False):
    thresholds = [1, 2, 4]
    ...
    for pcd_pred, pcd_gt in zip(pcd_pred_list, pcd_gt_list):
        for j, threshold in enumerate(thresholds):
            ...
            for i, cls in enumerate(occ_class_names):
                ...
                # flow L2 error
                if with_ave and (cls in flow_class_names and tp_mask.sum() > 0):
...
def main(sem_pred_list, sem_gt_list, lidar_origin_list, occ_class_names, flow_pred_list=None, flow_gt_list=None):
    ...
    if flow_pred_list is not None:
        for sem_pred, sem_gt, lidar_origins, flow_pred, flow_gt in tqdm(zip(sem_pred_list, sem_gt_list, lidar_origin_list, flow_pred_list, flow_gt_list), ncols=50):
            sem_pred = torch.from_numpy(np.reshape(sem_pred, [200, 200, 16]))
            sem_gt = torch.from_numpy(np.reshape(sem_gt, [200, 200, 16]))
            flow_pred = torch.from_numpy(np.reshape(flow_pred, [200, 200, 16, 2]))
            flow_gt = torch.from_numpy(np.reshape(flow_gt, [200, 200, 16, 2]))

            pcd_pred = process_one_sample(sem_pred, lidar_rays, lidar_origins, occ_class_names, flow_pred=flow_pred)
            pcd_gt = process_one_sample(sem_gt, lidar_rays, lidar_origins, occ_class_names, flow_pred=flow_gt)
        ...
        for sem_pred, sem_gt, lidar_origins in tqdm(zip(sem_pred_list, sem_gt_list, lidar_origin_list), ncols=50):
            ...
            pcd_pred = process_one_sample(sem_pred, lidar_rays, lidar_origins, occ_class_names)
            pcd_gt = process_one_sample(sem_gt, lidar_rays, lidar_origins, occ_class_names)
    ...
    iou_list, ave_list = calc_metrics(pcd_pred_list, pcd_gt_list, occ_class_names, with_ave=flow_pred_list is not None)
    rayiou = np.nanmean(iou_list)
    rayiou_0 = np.nanmean(iou_list[0])
    rayiou_1 = np.nanmean(iou_list[1])
    rayiou_2 = np.nanmean(iou_list[2])

    if ave_list is not None:
        ...
        table = PrettyTable([
            'Class Names',
            'RayIoU@1', 'RayIoU@2', 'RayIoU@4', 'AVE'
        ])
        ...
        table.add_row(['MEAN', rayiou_0, rayiou_1, rayiou_2, mave])
