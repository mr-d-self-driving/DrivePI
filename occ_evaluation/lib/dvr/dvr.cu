...
#include <string>
#include <iostream>
#include <float.h> // 添加这行以支持DBL_MAX
...
std::vector<torch::Tensor> render_forward_cuda(
    ...
    //         }));
        ...
        // 修改这部分代码，使用新的分发方式
    if (sigma.scalar_type() == torch::ScalarType::Float) {
        render_forward_cuda_kernel<float><<<blocks, threads>>>(
            sigma.packed_accessor32<float,5,torch::RestrictPtrTraits>(),
            origin.packed_accessor32<float,3,torch::RestrictPtrTraits>(),
            points.packed_accessor32<float,3,torch::RestrictPtrTraits>(),
            tindex.packed_accessor32<float,2,torch::RestrictPtrTraits>(),
            pred_dist.packed_accessor32<float,2,torch::RestrictPtrTraits>(),
            gt_dist.packed_accessor32<float,2,torch::RestrictPtrTraits>(),
            coord_index.packed_accessor32<float,3,torch::RestrictPtrTraits>(),
            train_phase);
    } else if (sigma.scalar_type() == torch::ScalarType::Double) {
        render_forward_cuda_kernel<double><<<blocks, threads>>>(
            sigma.packed_accessor32<double,5,torch::RestrictPtrTraits>(),
            origin.packed_accessor32<double,3,torch::RestrictPtrTraits>(),
            points.packed_accessor32<double,3,torch::RestrictPtrTraits>(),
            tindex.packed_accessor32<double,2,torch::RestrictPtrTraits>(),
            pred_dist.packed_accessor32<double,2,torch::RestrictPtrTraits>(),
            gt_dist.packed_accessor32<double,2,torch::RestrictPtrTraits>(),
            coord_index.packed_accessor32<double,3,torch::RestrictPtrTraits>(),
            train_phase);
    } else {
        AT_ERROR("render_forward_cuda not implemented for ", sigma.scalar_type());
    }
...
std::vector<torch::Tensor> render_cuda(
    ...
    // 修改这部分代码，使用新的分发方式
    if (sigma.scalar_type() == torch::ScalarType::Float) {
        render_cuda_kernel<float><<<blocks, threads>>>(
            sigma.packed_accessor32<float,5,torch::RestrictPtrTraits>(),
            origin.packed_accessor32<float,3,torch::RestrictPtrTraits>(),
            points.packed_accessor32<float,3,torch::RestrictPtrTraits>(),
            tindex.packed_accessor32<float,2,torch::RestrictPtrTraits>(),
            pred_dist.packed_accessor32<float,2,torch::RestrictPtrTraits>(),
            gt_dist.packed_accessor32<float,2,torch::RestrictPtrTraits>(),
            grad_sigma.packed_accessor32<float,5,torch::RestrictPtrTraits>(),
            loss_type);
    } else if (sigma.scalar_type() == torch::ScalarType::Double) {
        render_cuda_kernel<double><<<blocks, threads>>>(
            sigma.packed_accessor32<double,5,torch::RestrictPtrTraits>(),
            origin.packed_accessor32<double,3,torch::RestrictPtrTraits>(),
            points.packed_accessor32<double,3,torch::RestrictPtrTraits>(),
            tindex.packed_accessor32<double,2,torch::RestrictPtrTraits>(),
            pred_dist.packed_accessor32<double,2,torch::RestrictPtrTraits>(),
            gt_dist.packed_accessor32<double,2,torch::RestrictPtrTraits>(),
            grad_sigma.packed_accessor32<double,5,torch::RestrictPtrTraits>(),
            loss_type);
    } else {
        AT_ERROR("render_cuda not implemented for ", sigma.scalar_type());
    }
...
torch::Tensor init_cuda(
    ...
    // 修改这部分代码，使用新的分发方式
    if (points.scalar_type() == torch::ScalarType::Float) {
        init_cuda_kernel<float><<<blocks, threads>>>(
            points.packed_accessor32<float,3,torch::RestrictPtrTraits>(),
            tindex.packed_accessor32<float,2,torch::RestrictPtrTraits>(),
            occupancy.packed_accessor32<float,5,torch::RestrictPtrTraits>());
    } else if (points.scalar_type() == torch::ScalarType::Double) {
        init_cuda_kernel<double><<<blocks, threads>>>(
            points.packed_accessor32<double,3,torch::RestrictPtrTraits>(),
            tindex.packed_accessor32<double,2,torch::RestrictPtrTraits>(),
            occupancy.packed_accessor32<double,5,torch::RestrictPtrTraits>());
    } else {
        AT_ERROR("init_cuda not implemented for ", points.scalar_type());
    }
