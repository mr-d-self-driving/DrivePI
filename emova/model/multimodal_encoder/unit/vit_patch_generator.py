...
class ViTPatchGenerator(nn.Module):
    ...
    def _load_embed(self, src_embed: torch.Tensor, targ_embed: nn.Parameter):
        if src_embed.shape != targ_embed.shape:
            ...
            src_embed = F.interpolate(src_embed, size=(self.num_rows, self.num_cols), mode='bicubic',
    ...
    def _load_projection(self, src_proj_weight: torch.Tensor, targ_proj_weight: torch.Tensor):
        if src_proj_weight.shape != targ_proj_weight.shape:
            ...
            src_proj_weight = rearrange(src_proj_weight, 'b (c h w) -> b c h w', c=3, h=src_patch_size,
                                        w=src_patch_size)
            src_proj_weight = F.interpolate(src_proj_weight, size=(self.patch_size, self.patch_size), mode='bicubic',
                                            align_corners=True, antialias=False)
    ...
    def _get_pos_embeddings(self, batch_size: int, input_dims: Tuple[int, int]):
        ...
        if self.cpe_mode:
            if self.training:
                ...
                pos_embed = F.interpolate(pos_embed.float(), size=(max_dim, max_dim), align_corners=True,
...
class ViTPatchLinear(nn.Linear):
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict, missing_keys, unexpected_keys,
        ...
        if chk_weight.shape != self.weight.shape:
            ...
            chk_weight = F.interpolate(chk_weight, size=(self.patch_size, self.patch_size), mode='bicubic',
                                       align_corners=True, antialias=False)
