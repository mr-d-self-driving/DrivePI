...
def collate_fn(batches):
    ...
    input_ids = torch.stack([_['input_ids'] for _ in batches], 0)
    image_tensors = torch.stack([_['image_tensors'] for _ in batches], 0)

    return input_ids, image_tensors, image_sizes, prompts, targets, questions_ids

class JsonDataset(torch.utils.data.Dataset):

    def __init__(self, data_subset, tokenizer, conv_mode, image_processor, model_config):
    ...
    def __getitem__(self, index):
        ...
        image_tensor, image_sizes = process_images([image], self.image_processor, self.model_config)
...
if __name__ == '__main__':
    ...
    dataset = JsonDataset(args.data_subset, tokenizer, args.conv_mode, image_processor, model.config)
    ...
    for _, (input_ids, image_tensors, image_sizes, prompts, targets, questions_ids) in tqdm(enumerate(dataloader), total = len(dataloader)):

        with torch.inference_mode():
            output_ids = model.generate(
                input_ids.to(device=device, non_blocking=True),
                images=image_tensors.half().to(device=device, non_blocking=True),
