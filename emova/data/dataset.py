    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            ...
            if self.data_args.image_aspect_ratio == 'pad':
                ...
                images_size = [image.size for image in images]
                images = [process_anyres_image(  # torch.Size([5, 3, 336, 336])
                    image, processor, self.data_args.image_grid_pinpoints) for image in images]
                ...
                try:
                    ...
                    print(e, sources)
                    return self.__getitem__(random.randint(0, len(self) - 1))
                images = inputs['pixel_values']  # [ grid_H * grid_W, channel(1176) ]
                images_size = inputs['image_grid_thw']  # [ 1, grid_H, grid_W ]
            else:
                images_size = [image.size for image in images]
                images = processor.preprocess(images, return_tensors='pt')['pixel_values']
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in instances[0]:
            ...
            if self.image_aspect_ratio == "native_anyres":
                ...
                batch['image_sizes'] = torch.concat(image_sizes)
            else:
                if all(x is not None and x.shape[-2:] == images[0].shape[-2:] for x in images):
                    batch['images'] = torch.concat(images)
                else:
                    batch['images'] = images
